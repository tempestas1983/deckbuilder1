...
Status: v0.1.37 (frontend-engineer) — 2026-08-06, **NOCH NICHT committet**
(vom documenter in dieser Sweep-Session gegen den tatsächlichen Code
verifiziert — `Read`/`Grep` über `src/ui/store.ts`, `src/ui/render.ts`,
`src/ui/components/graveyardStack.ts`, `src/ui/style.css` sowie beide neuen
Testdateien, kein eigener Shell-Zugriff für einen echten Testlauf, s.
Verifikations-Hinweis im v0.1.37-Abschnitt unten). Davor v0.1.36
(2026-08-05), zum Zeitpunkt dieses Sweeps ebenfalls weiterhin uncommitted, s.
dortiger Abschnitt. Davor v0.1.35 (2026-08-02), zum Zeitpunkt dieses Sweeps
ebenfalls weiterhin uncommitted, s. dortiger Abschnitt.
...
**v0.1.37 auf einen Blick** (Details im gleichnamigen Abschnitt unten,
2026-08-06, NOCH NICHT committet): zwei voneinander unabhängige
Frontend-Features derselben Session. **1. Auto-Tap-Komfort.** Löst den seit
`docs/rules-engine.md` 9.5 dokumentierten „kommt später"-Punkt UND einen real
gemeldeten Bug (manuelles Tappen konnte 2 gleichfarbige Quellen für eine
Karte verbrauchen, die nur 1 Pip + 1 generisch brauchte, und dadurch eine
zweite identische Karte unbezahlbar machen, obwohl das Gesamt-Mana gereicht
hätte). Neue `store.ts`-Funktionen `hypotheticalStateWithExtraMana`
(gemeinsam mit dem bestehenden `hasRealPriorityChoice` genutzt),
`castCandidatesForHandCard`, `AUTO_TAP_COLOR_ORDER`, `manaAbilityColor`,
`selectAutoTapSources`, `autoTapActionsForCast`, verdrahtet in `dispatch()`:
farbige Pips werden zuerst aus gleichfarbigen Quellen gedeckt, generische
Kosten bevorzugt aus farblosen Quellen (mirrort `engine/mana.ts#payCost`,
rückwärts auf die Quellenauswahl angewandt). Nur menschliche Spieler,
Bot-Entscheidungslogik unverändert; X-Kosten-Zauber bewusst ausgenommen
(aktuell kein Kartenpool-Fall). Neuer Test `auto-tap-mana.test.ts`. **2.
Friedhof einklappen.** Ein nicht-leerer Friedhof zeigt jetzt EINE
eingeklappte Stapel-Kachel (neue Datei `components/graveyardStack.ts#
graveyardStackTile`, echtes Artwork der obersten Karte statt Kartenrücken —
der Friedhof ist öffentliche Information — plus Zahl-Badge, reuse von
`.hand-card-hidden-stack-multi`/`-count`) statt jeder Karte einzeln in voller
Größe; Klick öffnet ein scrollbares Popover (`graveyardPopoverPanel`, reuse
des `.tutorial-help-backdrop`/`-panel`-Musters von `keywordGlossaryPanel`)
mit der vollständigen Liste. Leerer Friedhof unverändert. Neue
Store-Exporte `openGraveyardPopoverPlayer`/`getOpenGraveyardPopover`/
`toggleGraveyardPopover`/`closeGraveyardPopover`. Neuer Test
`graveyard-stack.test.ts`.
...
11. **Auto-Tap-Komfort** (**neu in v0.1.37**, s. eigener Abschnitt unten):
    ist eine Handkarte aus dem aktuellen Manapool allein noch nicht
    bezahlbar, aber mit den eigenen ungetappten Mana-Fähigkeiten schon,
    bietet die Handkarte trotzdem einen „Spielen"-Button an; ein Klick
    tappt automatisch genau die dafür nötigen Quellen (farbige Pips zuerst
    aus gleichfarbigen Quellen, generische Kosten bevorzugt aus farblosen
    Quellen) und castet danach im selben Vorgang. Behebt einen real
    gemeldeten Bug: manuelles Vor-Tappen konnte zwei gleichfarbige Quellen
    für eine Karte verbrauchen, die nur einen Pip + eine generische Einheit
    brauchte, und dadurch eine zweite identische Karte unbezahlbar machen.
    Nur menschliche Spieler betroffen, X-Kosten-Karten bewusst ausgenommen.
12. **Friedhof einklappen** (**neu in v0.1.37**, s. eigener Abschnitt
    unten): ein nicht-leerer Friedhof zeigt jetzt EINE eingeklappte
    Stapel-Kachel (echtes Artwork der obersten/zuletzt hinzugekommenen
    Karte + Zahl-Badge ab 2 Karten, der Friedhof ist öffentliche
    Information) statt jeder Karte einzeln in voller Größe. Klick öffnet
    ein scrollbares Popover mit der vollständigen Liste, Schließen per
    „Schließen"-Button oder Klick auf den Hintergrund. Leerer Friedhof
    unverändert.
...
## Auto-Tap-Komfort + Friedhof einklappen (v0.1.37, 2026-08-06, NOCH NICHT committet)

Zwei voneinander unabhängige Frontend-Features derselben Session, keine
Engine-/Model-/Kartenpool-/KI-Änderung. Gegen den tatsächlichen Code gelesen
(`Read`/`Grep`, nicht nur den Fertigstellungsbericht übernommen):
`src/ui/store.ts` (vollständiger Auto-Tap-Abschnitt inkl. `dispatch()`-
Verdrahtung und Friedhof-Popover-Zustand), `src/ui/render.ts` (`handZone`/
`graveyardZone`), neue Datei `src/ui/components/graveyardStack.ts`
(vollständig gelesen), `src/ui/style.css` (neue Klassen), sowie beide neuen
Tests `src/ui/__tests__/auto-tap-mana.test.ts`/`graveyard-stack.test.ts`
(vollständig gelesen).

### 1. Auto-Tap-Komfort

Löst den seit `docs/rules-engine.md` Abschnitt 9.5 dokumentierten
„kein Auto-Tap ... kommt später"-Punkt (jetzt dort entsprechend aktualisiert,
s. dieser Sweep). Behebt zusätzlich einen real gemeldeten Bug: ein Spieler,
der Manaquellen manuell einzeln antippt, konnte für eine Karte mit Kosten
`{generic:1, void:1}` versehentlich BEIDE eigenen Void-Quellen tappen (1 Pip
+ 1 generisch aus der zweiten Void-Quelle statt aus einer generischen/
anderen Quelle) — bei zwei identischen Handkarten blieb dann keine
Void-Quelle mehr für die zweite Karte übrig, obwohl insgesamt genug
Gesamt-Mana vorhanden war.

- **`store.ts#hypotheticalStateWithExtraMana(player, extraMana)`** (neu,
  modul-intern): klont `state` einmalig mit einem lokal um `extraMana`
  erhöhten Manapool von `player` — reiner, seiteneffektfreier Hilfsklon für
  hypothetische `getLegalActions`-Aufrufe, nie persistiert.
  **Refactoring:** dieser Klon-Code existierte vorher bereits inline in
  `hasRealPriorityChoice` (v0.1.31-Bugfix) — jetzt einmal extrahiert und von
  BEIDEN Stellen gemeinsam genutzt, statt einer zweiten, unabhängigen Kopie
  für dieses Feature. `hasRealPriorityChoice` selbst ist dadurch nur
  strukturell verändert (ruft jetzt die extrahierte Funktion auf), sein
  beobachtbares Verhalten bleibt unverändert.
- **`store.ts#castCandidatesForHandCard(player, cardInstanceId, actions)`**
  (neu, exportiert): liefert für eine Handkarte `castSpell`-Kandidaten, die
  entweder aus dem aktuellen Manapool schon bezahlbar sind (Normalfall,
  identisch zur bisherigen Filterung in `render.ts`) ODER es WÜRDEN, wenn
  zusätzlich alle eigenen ungetappten Mana-Fähigkeiten aktiviert würden
  (gleiche hypothetische-Pool-Technik wie `hasRealPriorityChoice`). Reine
  Anzeige-Entscheidung ohne Seiteneffekt — das tatsächliche Antippen
  passiert erst beim Klick auf „Spielen", nicht hier. `render.ts#handZone`
  ruft diese Funktion jetzt statt einer rohen `candidates.filter(...)` auf.
- **`store.ts#selectAutoTapSources(cost, costDelta, currentPool,
  manaAbilityActions)`** (neu, modul-intern): der eigentliche Auswahl-
  Algorithmus. 1. Für jede farbige Anforderung, die der aktuelle Pool noch
  nicht deckt, wird pro fehlendem Pip genau eine ungetappte Quelle DIESER
  Farbe getappt. 2. Für die verbleibenden generischen Kosten werden zuerst
  ungetappte FARBLOSE Quellen getappt, erst danach — falls nicht genug —
  weitere Quellen in fester Reihenfolge (`AUTO_TAP_COLOR_ORDER = ["flame",
  "tide", "wild", "light", "void"]`, identisch zur internen `COLORS`-
  Reihenfolge in `engine/mana.ts`). Mirrort bewusst das Prinzip aus
  `engine/mana.ts#payCost` („generische Kosten zuerst aus Farblos"), nur
  rückwärts auf die Auswahl der zu tappenden QUELLEN statt auf den Verbrauch
  eines bereits gefüllten Pools angewandt — rein gierige Auswahl, keine
  kombinatorische Suche (jede Quelle produziert im aktuellen Kartenpool eine
  feste Farbe). Gibt `undefined` zurück, falls selbst alle Quellen die
  Kosten nicht decken würden (praktisch nie erreicht, weil
  `castCandidatesForHandCard` dieselbe Erreichbarkeit vorab prüft, bevor der
  „Spielen"-Button überhaupt erscheint).
- **`store.ts#autoTapActionsForCast(action)`** (neu, modul-intern): liefert
  für einen `castSpell`-Kandidaten die vor dem Cast automatisch
  auszuführenden `activateAbility`-Aktionen — `[]`, wenn der Pool die Kosten
  bereits allein deckt, die Karte X-Kosten hat (bewusst nicht unterstützt,
  aktuell kein Kartenpool-Fall), oder selbst mit allen Quellen nicht genug
  Mana zusammenkäme.
- **Verdrahtung in `dispatch()`:** ist die eingehende Aktion ein
  `castSpell`, der aus dem Pool allein nicht bezahlbar wäre, aber mit den
  automatisch ermittelten Quellen schon, werden erst die Tap-Aktionen
  angewendet (jede einzeln über `engine.applyAction`, `processEvents` je
  Schritt) und danach der eigentliche Cast — als EIN durchgehender Vorgang,
  `notify()`/`triggerAutomation()` bewusst erst NACH dem gesamten Ablauf
  (kein Zwischen-Rendern zwischen einzelnen Taps). Schlägt einer der
  Tap-Schritte defensiv fehl (sollte praktisch nie eintreten), bricht
  `dispatch()` sauber ab, bevor der Cast versucht wird.
- **Bot-Spieler sind NICHT betroffen** — `dispatch()` ist laut
  Funktionskommentar nur für menschliche Klicks; automatische KI-Züge laufen
  weiterhin über `runBotStep`/`src/ai/*.ts` mit deren eigener, unveränderter
  Tap-Entscheidungslogik.
- **Neuer Test `src/ui/__tests__/auto-tap-mana.test.ts`** (1 Fall, per
  Code-Lektüre bestätigt: reproduziert exakt das gemeldete Szenario — 2 Void
  + 2 Flame ungetappt, zwei Handkarten mit je `{generic:1, void:1}`. Ein
  Klick auf „Spielen" pro Karte reicht (Stack-Autopass löst den Rest auf),
  bestätigt nach der ersten Karte GENAU 1 getappte Void- + 1 getappte
  Flame-Quelle (nicht beide Void-Quellen), nach der zweiten Karte beide
  Void- + beide Flame-Quellen — beide Karten bleiben castbar).

### 2. Friedhof einklappen

Nutzerbericht (per Auftrag): „Friedhof zeigt jede Karte einzeln in voller
Größe, verschwendet viel Platz für eine Zone, die selten im Detail
inspiziert wird."

- **Neue Datei `src/ui/components/graveyardStack.ts`:**
  - `graveyardStackTile(state, pool, topInstanceId, count, onClick)` — für
    einen NICHT-leeren Friedhof: eine volle `cardTile()` der obersten
    (zuletzt hinzugekommenen) Karte, plus Zahl-Badge sobald `count > 1`.
    Zeigt bewusst das ECHTE Kartenbild statt eines Kartenrückens (anders als
    die verdeckte Hand, s. v0.1.35 `hiddenHandStack`) — der Friedhof ist,
    anders als die Hand, öffentliche Information (jeder Spieler darf
    jederzeit hineinsehen). Reuse der Stapel-Optik-CSS-Klassen
    `.hand-card-hidden-stack-multi`/`.hand-card-hidden-stack-count` aus der
    v0.1.35-Hand-Stapel-Kachel — bewusst wiederverwendet statt eine
    zusätzliche, fast identische zweite Optik neu zu bauen.
  - `graveyardPopoverPanel(state, pool, cardInstanceIds, title, onClose)` —
    Backdrop + Panel mit der vollständigen Liste (neueste zuerst, spiegelt
    die Kachel), volle `cardTile()`-Größe in einem scrollbaren Grid, keine
    Klick-/Ziel-Interaktion auf den einzelnen Karten (der Friedhof hat
    aktuell keine Einzelkarten-Interaktion). Struktureller Zwilling von
    `components/keywordGlossaryPanel.ts` (`.tutorial-help-backdrop`/
    `-panel`-Muster, Klick auf den Backdrop schließt, Klick INS Panel stoppt
    die Propagation).
- **`render.ts#graveyardZone`** (umgebaut): leerer Friedhof bleibt
  UNVERÄNDERT der bisherige `.empty-hint`-Text; ein nicht-leerer Friedhof
  rendert jetzt NUR NOCH `graveyardStackTile` (statt vorher jeder Karte
  einzeln). Die bestehende „frisch verstorben"-Juice-Einblende (v0.1.35,
  `getJuiceCardEffect(topId) === "death"`) bleibt an der eingeklappten
  Kachel erhalten, solange die zuletzt hinzugekommene Karte genau die
  oberste ist (Normalfall — aktuell gibt es keinen Effekt, der eine ANDERE
  als die zuletzt bewegte Karte im Friedhof anfasst).
- **Neuer Store-Zustand** (`openGraveyardPopoverPlayer`, modul-intern) +
  Exporte `getOpenGraveyardPopover()`/`toggleGraveyardPopover(playerId)`/
  `closeGraveyardPopover()` — ein Klick auf die Stapel-Kachel toggelt das
  Popover für GENAU diesen Spieler (Klick auf die eigene Kachel bei bereits
  offenem Popover schließt es wieder). Wird wie der übrige transiente
  UI-Zustand bei `initGame`/`resumeSavedGame` zurückgesetzt.
- **Verdrahtung in `renderGameBoard`** (analog zu
  `isKeywordGlossaryPanelOpen`): ist `getOpenGraveyardPopover()` gesetzt,
  wird `graveyardPopoverPanel` zusätzlich über dem Spielbrett gerendert,
  `onClose` ruft `closeGraveyardPopover()`.
- **Neuer Test `src/ui/__tests__/graveyard-stack.test.ts`** (2 Fälle, per
  Code-Lektüre bestätigt): leerer Friedhof zeigt weiterhin unverändert nur
  den Leer-Hinweis (kein Stapel, kein Popover); ein Friedhof mit 2
  unterschiedlichen Karten (über den bestehenden Cleanup-Zwangsabwurf
  kontrolliert erzeugt) zeigt genau EINE Stapel-Kachel mit Zahl-Badge „2",
  Klick öffnet ein Popover mit beiden Kartennamen und exakt 2
  `.card-tile`-Einträgen im Grid, Schließen funktioniert sowohl über den
  „Schließen"-Button als auch über einen Klick auf den Backdrop.

### Verifikation

**Vom documenter in dieser Sweep-Session direkt gegen den tatsächlichen Code
gegengeprüft** (nicht nur der Auftrags-Bericht übernommen): jede genannte
Funktion/Konstante/Datei wurde per `Read`/`Grep` im tatsächlichen
`src/ui/store.ts`/`src/ui/render.ts`/`src/ui/components/graveyardStack.ts`/
`src/ui/style.css` sowie in beiden neuen, vollständig gelesenen Testdateien
verifiziert — keine Abweichung vom Bericht gefunden. Testzahlen zusätzlich
per `Glob` gegen den tatsächlichen Dateibaum gegengezählt: exakt **53
Testdateien** (19 Engine + 7 KI + 27 UI, davon die zwei neuen
`auto-tap-mana.test.ts`/`graveyard-stack.test.ts`, je per Grep auf 1 bzw. 2
`it(`-Fälle gegengezählt). Rechnerische Gegenprobe gegen den vorherigen
dokumentierten Stand: 229 (v0.1.36-Stand, 51 Dateien) + 1
(`auto-tap-mana.test.ts`) + 2 (`graveyard-stack.test.ts`) = **232** —
stimmt exakt mit dem gemeldeten `npx vitest run`-Ergebnis überein (**52
Testdateien grün + 1 bewusst übersprungen (53 gesamt), 232 Tests grün + 1
bewusst übersprungen (233 gesamt)**). `npx tsc --noEmit` laut Auftrag
sauber. **Nicht selbst nachvollzogen (kein Shell-Werkzeug in dieser
Sweep-Session):** der eigentliche `npx tsc`/`npx vitest run`-Lauf selbst
sowie der „NOCH NICHT committet"-Stand — beide laut Auftrag bereits real
ausgeführt/geprüft, hier ohne eigene Shell-Gegenprobe übernommen (identische
Einschränkung wie bei allen Sweeps seit 2026-07-18).

**Geänderte/neue Dateien laut Code-Lektüre:** `src/ui/store.ts`
(`hypotheticalStateWithExtraMana`/`castCandidatesForHandCard`/
`AUTO_TAP_COLOR_ORDER`/`manaAbilityColor`/`selectAutoTapSources`/
`autoTapActionsForCast`, Verdrahtung in `dispatch()`;
`openGraveyardPopoverPlayer`/`getOpenGraveyardPopover`/
`toggleGraveyardPopover`/`closeGraveyardPopover`), `src/ui/render.ts`
(`handZone` ruft `castCandidatesForHandCard`; umgebaute `graveyardZone` +
Popover-Verdrahtung), `src/ui/style.css` (neue `.graveyard-stack`/
`.graveyard-popover-panel`/`.graveyard-popover-grid`). Neu:
`src/ui/components/graveyardStack.ts`, `src/ui/__tests__/
auto-tap-mana.test.ts`, `src/ui/__tests__/graveyard-stack.test.ts`. Keine
Engine-/Modell-/Kartenpool-/KI-Änderung. `docs/rules-engine.md` Abschnitt 9.5
wurde in diesem Sweep aktualisiert (der „Auto-Tap ... kommt später"-Satz ist
jetzt als erledigt markiert, mit Verweis hierher) — die einzige
...
25. **v0.1.36 committen** (s. eigener Abschnitt oben, „Spielspeicher in der
    ...
    verfügbar).
26. **v0.1.37 committen** (s. eigener Abschnitt oben, „Auto-Tap-Komfort +
    Friedhof einklappen") — zum Zeitpunkt dieses Sweeps laut Auftrag noch
    NICHT committet; bitte bei nächster Gelegenheit per `git status`/
    `git log` gegenprüfen und diesen Hinweis dann analog zu Punkt 25
    auflösen (zusammen mit v0.1.35/v0.1.36, s. dortige Punkte, sofern die
    noch offen sind).
27. **Auto-Tap-Komfort deckt X-Kosten-Karten bewusst nicht ab** (v0.1.37) —
    aktuell kein Kartenpool-Fall, aber falls künftig eine bezahlbare
    X-Kosten-Karte mit ansonsten unzureichendem Pool hinzukommt, bleibt der
    „Spielen"-Button für sie unverändert nur bei vollständig manuell
    vorgetapptem Pool sichtbar (keine Auto-Tap-Unterstützung für den
    variablen X-Anteil).
28. **Echte Browser-/Screenshot-Verifikation von Auto-Tap-Komfort und dem
    Friedhof-Popover steht aus** (v0.1.37) — beide bisher nur per
    jsdom-Klick-Test verifiziert, kein Browser-/Computer-Use-Werkzeug in
    dieser Sweep-Session verfügbar.
