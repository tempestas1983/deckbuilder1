...
## Aktueller Stand (2026-08-06, Regelwerk v0.3.3 / Modell v0.2.1 mit v0.3-Erweiterungen / Engine v0.3.6 / Kartenpool 300 Karten + 3 Token-Definitionen / Frontend v0.1.37 / KI-Gegner v2.3, **die letztgenannten Schritte (Frontend v0.1.35+v0.1.36+v0.1.37, KI v2.3 inkl. des darin bereits enthaltenen v2.2-Lethal-Checks) noch uncommitted**)
...
| UI (Spielbrett, Vite + TypeScript) | `src/ui/*`, Status: `docs/frontend-status.md` | **v0.1.37, NOCH NICHT committet** — auf v0.1.36 folgten zwei weitere, voneinander unabhängige Frontend-Features (2026-08-06): **(1) Auto-Tap-Komfort** löst den seit `docs/rules-engine.md` 9.5 dokumentierten „kommt später"-Punkt UND einen real gemeldeten Bug (manuelles Tappen konnte 2 gleichfarbige Quellen für eine Karte verbrauchen, die nur 1 Pip + 1 generisch brauchte, und dadurch eine zweite identische Karte unbezahlbar machen) — `store.ts#selectAutoTapSources`/`autoTapActionsForCast`, verdrahtet in `dispatch()`, tappt vor einem sonst unbezahlbaren Cast automatisch die nötigen eigenen Mana-Fähigkeiten (farbige Pips zuerst gleichfarbig, generische Kosten bevorzugt farblos, mirrort `engine/mana.ts#payCost`); nur menschliche Spieler, X-Kosten-Karten bewusst ausgenommen. **(2) Friedhof einklappen**: ein nicht-leerer Friedhof zeigt jetzt EINE eingeklappte Stapel-Kachel (neue Datei `components/graveyardStack.ts#graveyardStackTile`, echtes Artwork statt Kartenrücken, da öffentliche Information) statt jeder Karte einzeln; Klick öffnet ein scrollbares Popover (`graveyardPopoverPanel`) mit der vollständigen Liste. Neue Tests `auto-tap-mana.test.ts`/`graveyard-stack.test.ts`. Keine Engine-/Model-/Kartenpool-/KI-Änderung (`docs/rules-engine.md` 9.5 wurde nur dokumentarisch aktualisiert, kein Engine-Code). Details: `docs/frontend-status.md` Abschnitt „Auto-Tap-Komfort + Friedhof einklappen (v0.1.37)". Davor (v0.1.36, s. vorige Sweep-Fassung): „Spielspeicher in der Partie" — ein einzelner, automatisch aktualisierter `localStorage`-Autosave-Slot (Key `deckbuilder1.savedGame`, `store.ts#autosaveGameForEvent`, gleiches Hook-Muster wie `recordGameHistoryForEvent`/`applyJuiceForEvent`), der eine laufende Partie nach jeder state-verändernden Aktion sichert (Tutorial-Partien ausgenommen) und über einen neuen, nur bei vorhandenem Autosave sichtbaren „Weiter spielen"-Button im Hauptmenü (`components/mainMenu.ts#resumeGameButton`) exakt an derselben Stelle fortsetzt (`store.ts#resumeSavedGame`, restauriert den `GameState` direkt statt erneut `createGame` aufzurufen — verifiziert: die `RulesEngine` ist eine reine state-in/state-out-Schnittstelle ohne verstecktem internen Zustand). Wird bei Spielende sowie beim Start einer neuen Partie automatisch überschrieben/gelöscht, kein Bestätigungsdialog, bewusst nur EIN Slot statt eines Mehrfach-Speichersystems. Neuer Test `game-save.test.ts` (4 Fälle). Details: `docs/frontend-status.md` Abschnitt „Spielspeicher in der Partie ... (v0.1.36)". Davor (v0.1.35, s. vorige Sweep-Fassung): fünf voneinander unabhängige Änderungen derselben Session (2026-08-02, Details in `docs/frontend-status.md`): **(1)** die verdeckte Gegner-Hand zeigt jetzt EINE Stapel-Kachel mit Zahl-Badge (`handCard.ts#hiddenHandStack`) statt einer Kartenrückseiten-Kachel PRO Handkarte; **(2)** Karten-Artwork wird jetzt als auf 480px verkleinertes WebP (Qualität 80, neue Dev-Dependency `sharp`) statt als rohes 1024px-PNG ausgeliefert (`vite.config.ts#cardArtworkPlugin` jetzt eigenständig statt über das generische `staticArtPlugin`, `cardArt.ts#artworkFileName` liefert `.webp`) — laut Auftrag schrumpft `dist-ui/cards/artworks/` dadurch von ~500 MB auf **7,9 MB**; **(3)** neuer, nur im Produktions-Build erzeugter Service Worker (`vite.config.ts#serviceWorkerPlugin` → `dist-ui/sw.js`, Runtime-Caching statt fester Precache-Liste, Registrierung in `main.ts` nur bei `import.meta.env.PROD`) macht die bereits vorhandene `manifest.json`/Icons jetzt echt offline-fähig; **(4)** neuer „Statistik"-Hauptmenü-Punkt (`AppPhase` `"stats"`, `components/statsScreen.ts`) zeigt einen dauerhaft in `localStorage` aufgezeichneten Spielverlauf (`store.ts#recordGameHistoryForEvent`, gedeckelt auf 100 Einträge, Tutorial-Partien ausgeschlossen); **(5)** neuer, von Musik/SFX unabhängiger „Effekte"-Toggle (`components/juiceToggle.ts`, `store.ts#isJuiceEnabled`/`applyJuiceForEvent`) für drei rein dekorative CSS-Animationen (Treffer-Zucken, Impact-Puls beim Auflösen eines Zaubers, Sterbe-Ausblenden), greift nur ohne `prefers-reduced-motion`; bestehende informationstragende Animationen bleiben bewusst ungekoppelt. Keine Engine-/Model-/Kartenpool-Änderung. Laut Auftrag (vom documenter mangels Shell-Zugriff nicht selbst nachgerechnet, aber die betroffenen Funktionen/Dateien einzeln gegen den Code gelesen und bestätigt): `npx tsc --noEmit` sauber, `npx vitest run` **48 Testdateien, 221 Tests grün, 1 bewusst übersprungen** (neue Dateien `game-history.test.ts`/`juice-toggle.test.ts`, je per Grep auf 2/4 `it(`-Fälle gegengezählt), `npx vite build` erfolgreich. Davor (v0.1.34, s. vorige Sweep-Fassung): auf v0.1.33 folgte eine weitere Session (2026-08-02, Details im „Seit dem letzten Sweep"-Absatz unten sowie `docs/frontend-status.md`): **Teil 1** ein zweiter Button „Weiter bis was passiert" neben dem bestehenden „Überspringen"-Button im Entscheidungs-Spotlight-Banner (`store.ts#passUntilSomethingHappens`) — passt für den auslösenden Spieler automatisch mehrfach hintereinander weiter, bis dessen eigene nächste Hauptphase erreicht ist oder ein neues Stack-Objekt auftaucht, OHNE `hasRealPriorityChoice`/`isRealPriorityCandidate` selbst zu ändern. **Teil 2** `jsdom` in `package.json` exakt auf `26.1.0` gepinnt — behebt einen ESM/CJS-Interop-Fehler (`jsdom@29.1.1` → `html-encoding-sniffer@6.0.0` → `require()` des reinen ESM-Pakets `@exodus/bytes`), der die komplette `src/ui/__tests__/`-Suite zuvor unausführbar gemacht hatte, plus ein von 15000ms auf 25000ms erhöhtes Timeout in `battlefield-grouping.test.ts` (Ressourcen-Kontention unter voller Suite-Last, kein Bug). Keine Engine-/Model-/Kartenpool-Änderung. Nach dem Fix lieferte ein echter `npx vitest run` **44 von 45 Testdateien / 213 von 214 Einzeltests grün** (1 Testdatei weiterhin bewusst per `describe.skip` übersprungen: `color-balance.analysis.test.ts`) — diese Zahl ersetzt die zuvor dokumentierten „177 Tests grün" als aktuellen Gesamtstand (Engine allein weiterhin 131, unverändert). Davor (v0.1.22–v0.1.33, s. vorige Sweep-Fassung): auf v0.1.21 folgten zwölf weitere Schritte, u. a. eine vertikale Zug-Flow-Spalte mit aktiver-Spieler-Avatar (v0.1.22), angereichertes Ereignis-Log + visuelles Action-Glow (v0.1.23, das Log-Panel selbst dann in v0.1.25 wieder entfernt, die Datenerfassung bleibt für Tests), direkt aneinanderstoßende Battlefields mit nach unten verschobener KI-Hand (v0.1.24), einstellbare Bot-Zuggeschwindigkeit (v0.1.25), Stack-Anzeige zwischen die Battlefields verschoben (v0.1.26), Archetyp-Deck-Auswahl für Mensch UND KI-Gegner (v0.1.27/v0.1.28), „Im Deck"-Abschnitt im Deckbau-Kartenpool (v0.1.29), ein Bugfix gegen wiederkehrendes Bild-Blinken bei Karten-/Avatar-Artwork (v0.1.30), ein kritischer Bugfix gegen automatisches Überspringen des Spielers direkt nach dem Terrain-Legen (v0.1.31), Battlefield-Typ-Gruppierung mit überlappenden Auren-Kacheln (v0.1.32) sowie eine deutlichere Hervorhebung der aktuellen Spielphase (v0.1.33). Keiner dieser zwölf Schritte hat Engine/Model/Kartenpool angefasst — unabhängig davon lieferte der engine-engineer zwischenzeitlich den oben in der Engine-Zeile beschriebenen v0.3.6-Bugfix. UI-Testdateien liefern laut `docs/frontend-status.md` (dort per echtem `npm test`-Lauf verifiziert, nicht nur Grep) **177 Tests grün + 1 bewusst übersprungener Analyse-Test** über Engine+UI+KI zusammen (Engine allein: 131). Frühere Schritte (bis v0.1.21) zur Referenz: **v0.1.17** die bis dahin mit Abstand umfangreichste Einzelsession — echtes **Hauptmenü** als neuer App-Einstiegspunkt (`AppPhase`: `mainMenu` → `opponentSelect` → `deckbuild` → `playing`, löst den bisherigen Direkteinstieg in den Deckbau ab) mit vier Optionen inkl. neuer Gegner-Auswahl (KI-Schwierigkeit mit benannten Bot-Personas „Ollo Wackelhand"/„Guntram Eichenfaust"/„Silas Kaltblick" ODER 2-Spieler-Hotseat) und neuem „Anleitung"-Nachschlage-Panel; dazu Taverne-Atmosphäre + Szenen-Artwork (viewport-breiter Board-Hintergrund, großformatige Gegner-Avatare in einer eigenen Spalte), sichtbare View-Transitions-Animationen (Karten morphen zwischen Zonen, Bot-Züge sichtbar statt Snap, Lebenspunkte-Puls), eine verdeckt dargestellte Gegner-Hand (nur Kartenrückseiten + Anzahl), zwölf Soundeffekte, sowie ein Tutorial-Bugfix (Hauptphase-Sperre gegen versehentliches Überspringen des Terrain-Schritts). **v0.1.18** Auto-Pass (automatisches Passen/„keine Angreifer"/„keine Blocker", wenn `getLegalActions` keine echte Wahl mehr bietet), ein auffälliges Entscheidungs-Spotlight-Banner für echte Entscheidungen, sowie eine von einer festen Datei auf Auto-Discovery umgestellte Musik-Playlist (`docs/music/` → `/music/index.json`, Titelauswahl + Wiederholungsmodus). **v0.1.19** Bugfix: reine Mana-Fähigkeiten (Terrain fürs Mana antippen) zählten fälschlich als „echte Entscheidung" und verhinderten Auto-Pass/lösten das Spotlight-Banner ständig unnötig aus. **v0.1.20** (Commit `9b81338`, beim vorigen Sweep noch uncommitted, jetzt bestätigt) benannte Deck-Speicherfunktion (mehrere Slots mit Name + Beschreibung), ein Deck-Analyse-Panel (Mana-Kurve, Farb-/Typverteilung) sowie ein „Deck leeren"-Button im Deckbau-Screen. **v0.1.21** (Commit `5654ec1`) neue Datei `aiDecks.ts`: 7 vom card-designer kuratierte Archetyp-Decklisten (je 60 Karten, echte Kopienzahlen/Kurve statt der bisherigen reinen 5-Farben-Zufallsmischung) — der Bot-Gegner zieht jetzt bei beiden automatischen Deckbau-Stellen (`render.ts`) über `pickRandomAiDeck()` eines dieser sieben Decks, `buildDemoDeck` bleibt unverändert für den „Zufällig füllen"-Button des menschlichen Deckbaus; der gewählte Archetyp wird dem Spieler bewusst nirgends angezeigt. Keiner dieser fünf Schritte (bis v0.1.21) hat Engine/Model/Kartenpool angefasst (v0.1.17 ergänzte nur die rein kosmetischen `BOT_DISPLAY_NAMES` in `src/ai/difficulty.ts`) — Stand zum damaligen v0.1.21-Sweep: Engine-Testzahl **130**, UI-Testdateien **19** einzeln benannte Fälle über 12 Dateien (neu seit v0.1.16: `golden-path.test.ts`, `main-menu.test.ts`, `rules-guide.test.ts`), per Grep plausibilisiert statt echtem Testlauf; **überholt durch die aktuellen Zahlen am Anfang dieser Zeile** (v0.1.22–v0.1.33: 131 Engine-Tests, 177 Gesamt+1 übersprungen, per echtem `npm test` je Session verifiziert) |
| KI-Gegner (drei Schwierigkeitsstufen, spielt ausschließlich über die öffentliche `RulesEngine`-Schnittstelle) | `src/ai/*`, Status: `docs/ai-status.md` | **v2.3, letzte beiden Schritte (v2.2+v2.3) uncommitted** — der in v1 (`simpleBot.ts`) als Fundament angelegte Bot wurde vom neuen `ai-opponent-engineer`-Subagenten zu drei echten Stufen ausgebaut: **easy** (`easyBot.ts`, absichtlich fehlerhaft/zufällig, aber immer regelkonform), **medium** (unverändert `simpleBot.ts`, die bisherige v1-Heuristik), **hard** (`hardBot.ts`/`boardEval.ts`, budgetiertes 1-Ply-Lookahead über echte `applyAction`-Simulation, effektive Stats/Keywords inkl. fremder Statics, echte Kampf-Mathematik/-Simulation). Deterministischer Stärkevergleich bestätigt strikte Stärkeordnung (medium schlägt easy, hard schlägt medium/easy, je >= 60 % der entschiedenen Partien). **v2.1:** zwei vom 300-Karten-Set aufgedeckte Legalitätsfehler behoben (Blocklegalität mit effektiven Keywords bei statisch gewährtem guardian; modale Kandidaten werden jetzt konsumentenseitig zu Modus×Ziel vervollständigt statt roh und ungültig eingereicht zu werden), plus ein neues, bewusst aus der CI ausgeschlossenes Analyse-Tool für Farb-Balance-Messungen (`color-balance.analysis.test.ts`, `describe.skip`, manuell mit `BALANCE_ANALYSIS=1` ausführbar) — dessen Befund hat die Balance-Korrekturrunden am Kartenpool ausgelöst (s. Zeile oben). Ein Engine-Bug (firstStrike-Token-Crash) wurde dabei gefunden und gemeldet, nicht selbst behoben — s. Engine-Zeile (v0.3.3). **v2.2 (2026-08-02, `docs/ai-status.md` Abschnitt 11):** neuer, gezielter Lethal-Check nur für `hard` (`hardBot.ts#findLethalAction`) — das isolierte 1-Ply-Lookahead übersah zuvor Züge, bei denen erst die Kombination aus Schadenszauber + Alpha-Strike diesen Zug lethal ist (jede Einzelaktion isoliert bewertet sah schlechter aus als z. B. eine Kreatur zu spielen); ein billiger Vorfilter ohne Simulation lässt nur in seltenen Fällen zwei vollständig durchsimulierte „Alles-rein"-Pläne starten (gegen ein „Gegner blockt gut"-Blockmodell, nicht optimistisch). `easy`/`medium` unverändert. Neuer Test `hardBot-lethal.test.ts`. **v2.3 (2026-08-02, `docs/ai-status.md` Abschnitt 12, NOCH NICHT committet, gleiche noch uncommittete `hardBot.ts`-Arbeitskopie wie v2.2):** zwei weitere, nur `hard` betreffende Heuristiken. (1) **Mana-Zurückhalten** (`shouldHoldManaBack`): im eigenen Main-Phase-Fenster tappt `hard` jetzt nicht mehr blind jede Manaquelle für eine nur marginal lohnende Option, wenn genau dadurch eine bezahlbare, „typischerweise reaktive" `fast`-Karte (Removal/Direktschaden) für den gegnerischen Zug unbezahlbar würde — nur der exakte Grenzfall „ungetappte Quellen == Kosten dieser Karte", klar lohnende Plays werden immer gespielt. (2) **2-Ply gegen billige Gegenantwort** (`evaluateCastCandidateEnd`): die bereits vorhandene Top-K-Cast-/Activate-Shortlist wird zusätzlich gegen bis zu zwei billige, statisch bestimmte Gegenantworten geprüft (Passen + die statisch beste gegnerische Option), falls nach dem eigenen Zug der Gegner Priorität behält — kein rekursiver Baum, Kosten K × (1-2) statt K × N. Laut Bericht des ai-opponent-engineer keine relevante Performance-Regression (Median 0,019 ms → 0,020 ms), Stärkevergleich unverändert (hard:medium 25:15, hard:easy 19:5). Neue Tests `hardBot-mana-hold-back.test.ts`/`hardBot-cast-reply.test.ts`. **Nachträglich per eigenem Testlauf bestätigt:** `npx vitest run src/ai` liefert 6 Testdateien grün + 1 übersprungen (7 gesamt), 26 Tests grün + 1 übersprungen (27 gesamt) — die „26" aus dem ursprünglichen Bericht war korrekt, bezog sich aber auf Tests statt Testdateien (Details: `docs/ai-status.md` Abschnitt 12.4) |
...
| Projekt-Setup (package.json, tsconfig, Vitest, Vite) | `package.json`, `tsconfig.json`, `vite.config.ts` | fertig — **2026-08-02 (v0.1.34):** `jsdom` in `package.json` exakt auf `26.1.0` gepinnt (statt einer gelockerten Bereichsangabe), behebt einen ESM/CJS-Interop-Fehler mit neueren `jsdom`-Versionen (`html-encoding-sniffer@6.0.0` bindet das reine ESM-Paket `@exodus/bytes` per `require()` ein), der die komplette `src/ui/__tests__/`-Suite unausführbar gemacht hatte, s. `docs/frontend-status.md` v0.1.34. **2026-08-02 (v0.1.35, NOCH NICHT committet):** neue Dev-Dependency `sharp@^0.35.3` (Karten-Artwork-Kompression, s. Frontend-Zeile oben), `vite.config.ts` um `cardArtworkPlugin` (jetzt eigenständig) + neues `serviceWorkerPlugin` erweitert (PWA-/Offline-Service-Worker) |
...
Regelwerk (v0.3.3), Engine (v0.3.6, 131 Engine-Tests), Frontend/KI
zusammen **232 Einzeltests grün + 1 bewusst übersprungener Analyse-Test,
über 52 von 53 Testdateien, über Engine+UI+KI** —
**documenter-Korrektur 2026-08-06 (v0.1.37-Sweep, „Auto-Tap-Komfort +
Friedhof einklappen"):** diese Zeile trug zuvor die v0.1.36-Zahl 229/229+1
über 50/51 Testdateien; der neue v0.1.37-Schritt (zwei neue UI-Testdateien,
`auto-tap-mana.test.ts` mit 1 Fall und `graveyard-stack.test.ts` mit 2
Fällen) hebt das laut Auftrag auf 232 grüne Einzeltests über 52 von 53
Testdateien (19 Engine + 7 KI + 27 UI) — per `Glob` gegen den tatsächlichen
Dateibaum gegengezählt (exakte Übereinstimmung), beide neuen UI-Testdateien
zusätzlich per Grep auf 1 bzw. 2 `it(`-Fälle verifiziert (s. „Seit dem
letzten Sweep (2026-08-05, Frontend v0.1.36)" unten), s. „Seit dem letzten
Sweep"-Absätze unten),
Starter-Kartenset (v0.15, 300 Karten + 3 Token-Definitionen), Spielbrett-UI
(**v0.1.37, NOCH NICHT committet**) und ein KI-Gegner mit drei echten
Schwierigkeitsstufen (**v2.3, ebenfalls NOCH NICHT committet**, s. „Seit dem
letzten Sweep (2026-08-05, Frontend v0.1.36)" unten für den neuesten
Frontend-Schritt bzw. weiter unten für die KI-Heuristiken) sind alle fertig und
end-to-end (Engine-seitig per echtem `npm test`,
...
**Seit dem letzten Sweep (2026-07-22) zusätzlich — zwei unabhängige
Frontend-Änderungen (v0.1.34, `docs/frontend-status.md`), keine
Engine-/Model-/Kartenpool-/KI-Änderung:**

- **„Weiter bis was passiert"-Button** (Nutzer-Feedback: musste bei JEDEM
  Priority-Fenster einzeln „Überspringen" klicken, nur weil eine Handkarte
  theoretisch castbar blieb, die gerade gar nicht gespielt werden sollte):
  neuer, zusätzlicher Button neben dem bestehenden „Überspringen"-Button im
  Entscheidungs-Spotlight-Banner (`components/decisionSpotlight.ts`, neuer
  Parameter `onSkipUntilSomethingHappens`). `store.ts#
  passUntilSomethingHappens(player)` startet einen einmaligen,
  spielerausgelösten Vorgang; `store.ts#
  shouldContinuePassingUntilSomethingHappens` hält ihn genau dann an, wenn
  entweder die eigene nächste Hauptphase (main1/main2) erreicht ist oder ein
  neues Stack-Objekt auftaucht, das vorher nicht da war. Ändert **nichts**
  an `hasRealPriorityChoice`/`isRealPriorityCandidate` selbst (ein castbarer
  Zauber bleibt weiterhin dauerhaft eine „echte Wahl", s. v0.1.18/v0.1.19
  oben) — das bestehende Sicherheitslimit gegen Endlosschleifen
  (`MAX_AUTO_HUMAN_ACTIONS_PER_CYCLE`) greift unverändert auch hier. Neuer
  Regressionstest `pass-until-something-happens.test.ts`.
- **UI-Testsuite (jsdom) repariert:** die komplette `src/ui/__tests__/`-Suite
  ließ sich vorher NICHT ausführen — `jsdom@29.1.1` zog
  `html-encoding-sniffer@6.0.0` (CJS), das wiederum das reine ESM-Paket
  `@exodus/bytes` per `require()` einband (ESM/CJS-Interop-Fehler). Fix:
  `package.json` pinnt `jsdom` jetzt exakt auf `26.1.0`. Zusätzlich wurde in
  `battlefield-grouping.test.ts` das Test-Timeout von 15000ms auf 25000ms
  erhöht (unter voller Suite-Last beobachtete Laufzeit ~21,5s statt ~12s
  isoliert — Ressourcen-Kontention, kein Bug).
- **Testzahlen (per echtem `npx vitest run` verifiziert, laut
  `docs/frontend-status.md`):** **44 von 45 Testdateien / 213 von 214
  Einzeltests grün**, 1 Testdatei weiterhin bewusst per `describe.skip`
  übersprungen (`color-balance.analysis.test.ts`, unverändert). `npm run
  build` (`tsc --noEmit`) sauber. Diese Zahl ersetzt die zuvor dokumentierten
  „177 Tests grün + 1 übersprungen" als aktuellen Gesamtstand — Engine allein
  weiterhin **131**, unverändert (keine Engine-Session in diesem Zeitraum).
- **documenter (dieser Sweep, 2026-08-02):** Beide Änderungen gegen den
  tatsächlichen Code gelesen (`src/ui/store.ts`,
  `src/ui/components/decisionSpotlight.ts`, `src/ui/render.ts`,
  `src/ui/style.css`, `package.json`, den neuen Test) statt den
  Fertigstellungsbericht blind zu übernehmen — Beschreibung der
  Stopp-Bedingungen von `shouldContinuePassingUntilSomethingHappens` (eigene
  nächste Hauptphase ODER neues Stack-Objekt) stimmt mit dem Code überein.
  **Ein gemeldeter Punkt erwies sich als falsch und wurde NICHT übernommen:**
  der Bericht behauptete, `src/cards/starter-set.ts` enthalte inzwischen 4
  Karten mit `isToken: true`, während die Doku noch 3 nenne. Per `Grep` gegen
  den tatsächlichen Code (`isToken: true` kommt exakt **3×** vor, dazu ein
  vierter Treffer nur als Kommentar auf derselben Textbasis wie schon beim
  Sweep vom 2026-07-18) bestätigt: es sind weiterhin **3** Token-Karten,
  macht weiterhin 300 reguläre Karten bei 303 `id: "core.…"`-Einträgen. Die
  Doku (`docs/README.md`, `docs/frontend-status.md`, `docs/cards/
  starter-set.md`) nannte an allen gefundenen Stellen bereits korrekt „3
  Token-Definitionen" — **keine Änderung nötig, keine der drei Dateien
  angefasst.** Eigener `npm test`/`npm run build`-Lauf in dieser
  Sweep-Session **nicht möglich** (kein Shell-Werkzeug verfügbar, wie bei
  allen Sweeps seit 2026-07-18) — die Zahlen 44/45 Testdateien und 213/214
  Einzeltests stammen aus dem im Auftrag beschriebenen echten
  `npx vitest run`; per `Glob`/`Grep` gegengeprüft: exakt 45 Testdateien
  (19 Engine + 4 KI + 22 UI) existieren im Repo, und kein einziges
  `it.skip`/`it.todo` kommt irgendwo vor — die einzige übersprungene Einheit
  ist die komplette, weiterhin unveränderte `color-balance.analysis.test.ts`
  (per Grep: genau 1 `it(`-Fall darin, passt exakt zur Differenz
  214 − 213 = 1). `docs/README.md` (Statustabelle, Kopfzeile, dieser
  Absatz), `docs/frontend-status.md` (Kopfzeile, neue „auf einen Blick"-
  Kurzfassung, neuer Detail-Abschnitt v0.1.34, Struktur-Tabelle,
  „Was funktioniert", „Nächste Schritte") auf den neuen Stand gehoben.
  `docs/status.md` bekam einen neuen, eigenen Sweep-Eintrag (s. dort).
  ...
  Engine-/Model-/Kartenpool-/KI-Änderung in dieser Session).

**Seit dem letzten Sweep (2026-08-02, Frontend v0.1.34) zusätzlich —
hard-Bot Lethal-Check (KI v2.2, `docs/ai-status.md` Abschnitt 11), NOCH
NICHT committet, keine Engine-/Model-/Kartenpool-/Frontend-Änderung:**

- **Lethal-Check nur für `hard`** (`src/ai/hardBot.ts#findLethalAction`,
  neuer Dispatch-Schritt zwischen Terrain-Spielen und der normalen
  1-Ply-Cast/Activate-Wahl): Das bestehende 1-Ply-Lookahead bewertet
  Cast-/Activate-/Attack-Kandidaten nur isoliert und konnte dadurch Züge
  übersehen, bei denen erst „Schadenszauber + Alpha-Strike zusammen"
  diesen Zug lethal ist — jede der beiden Einzelaktionen sah isoliert
  bewertet oft schlechter aus als z. B. eine größere Kreatur zu spielen.
  Fix: ein billiger, simulationsfreier Vorfilter (`mightBeLethalThisTurn`)
  lässt nur in den seltenen in-Frage-kommenden Fällen zwei vollständig
  durchsimulierte „Alles-rein"-Pläne starten (`"burnFirst"`/`"attackFirst"`,
  gegen ein „Gegner blockt gut"-Blockmodell, nicht optimistisch); findet
  einer davon einen echten Kill, wird dessen erster Schritt gespielt, sonst
  bleibt das bisherige 1-Ply-Verhalten unverändert. `easy`/`medium` bewusst
  unangetastet. Bewusste Grenze (wie X-Kosten, bestehendes Modul-Muster):
  X-Kosten-Zauber und Modi mit ≥ 2 Zielslots bleiben außen vor, weil
  `getLegalActions` sie generell nicht enumeriert.
- **Neuer Test** `src/ai/__tests__/hardBot-lethal.test.ts` — deterministische
  Falle (Gegner bei 5 Leben, genau 2 Mana, Schadenszauber vs. isoliert
  „bessere" Kreatur, nur die Kombination mit einem bereits vorhandenen
  Angreifer ist lethal).
- **Performance/Test-Zahlen laut Bericht des ai-opponent-engineer, vom
  ...
  Sweep-Session): Median-Entscheidungsdauer hard-vs-hard steigt von 0,011 ms
  auf 0,015 ms (Max weiterhin < 30 ms, weit unter der 1000-ms-CI-Schranke);
  `npx vitest run src/ai` weiterhin 22 grün + 1 bewusst übersprungen. Die
  Stärkevergleichs-Tallies für `hard` vs. `easy` weichen im Bericht leicht
  von der zuletzt dokumentierten v2.1-Baseline ab (19:5 statt 21:3, `hard`
  vs. `medium` deckungsgleich bei 25:15) — nachträglich aufgelöst: das 19:5-
  Tally stammt bereits aus einem Testlauf VOR dem Lethal-Check (jsdom-Fix am
  selben Tag), also vorbestehende Doku-Drift in der v2.1-Baseline, keine
  Auswirkung des Lethal-Checks selbst (Details: `docs/ai-status.md`
  Abschnitt 11.5).
- **documenter (dieser Mini-Sweep, 2026-08-02):** `docs/ai-status.md` (neue
  Kopfzeile + Abschnitt 11 + präzisierte Schwäche 1 in Abschnitt 9.7),
  `docs/README.md` (diese Kopfzeile, KI-Zeile der Statustabelle, dieser
  Absatz, „Weitere offene Punkte" Punkt 2 unten) sowie `docs/status.md`
  (neuer Sweep-Eintrag) aktualisiert — Code selbst (`src/ai/hardBot.ts` +
  neuer Test) gegen den tatsächlichen Diff gelesen, nicht nur den Bericht
  übernommen. `docs/rules-engine.md`, `docs/engine-status.md`,
  `docs/frontend-status.md`, `docs/cards/starter-set.md` waren nicht
  Gegenstand dieses Mini-Sweeps (keine Engine-/Model-/Kartenpool-/Frontend-
  Änderung).

**Seit dem letzten Sweep (2026-08-02, Frontend v0.1.34) zusätzlich —
fünf voneinander unabhängige Frontend-Änderungen (v0.1.35,
`docs/frontend-status.md`), NOCH NICHT committet, keine
Engine-/Model-/Kartenpool-/KI-Änderung:**

- **Kompaktere KI-Hand.** Die verdeckte Gegnerhand zeigte vorher pro
  ...
  (`components/handCard.ts#hiddenHandStack(hand)`, löst die alte
  Pro-Karte-Funktion ab) mit Zahl-Badge für die tatsächliche Handgröße und
  CSS-Stapel-Optik (`.hand-card-hidden-stack-multi::before`/`::after`, zwei
  versetzte Rückseiten-Flächen bei >1 Karte). Nur die GEGNERISCHE verdeckte
  Hand betroffen, die eigene offene Hand des Menschen unverändert.
- **Karten-Artwork-Kompression.** Die 303 Karten-Artworks
  (`docs/cards/artworks/`, gitignored/nutzer-gepflegt) waren 1024×1024-PNGs,
  laut Auftrag ~500 MB insgesamt, obwohl nie größer als 158px CSS-Breite
  gerendert. `vite.config.ts#cardArtworkPlugin` (jetzt eigenständig statt
  über das generische `staticArtPlugin`) transformiert im Dev-Server
  (on-the-fly, gecacht unter `node_modules/.cache/card-artwork-webp/`) UND
  beim Produktions-Build auf 480px WebP, Qualität 80, über die neue
  Dev-Dependency `sharp@^0.35.3`. Originaldateien bleiben unverändert,
  `cardArt.ts#artworkFileName` liefert jetzt `.webp` statt `.png`. Laut
  Auftrag schrumpft `dist-ui/cards/artworks/` dadurch von ~500 MB auf
  **7,9 MB** (~63×).
- **PWA-/Offline-Fähigkeit.** `public/manifest.json` + Icons existierten
  bereits (App war installierbar), es fehlte der Service Worker. Neu:
  `vite.config.ts#serviceWorkerPlugin`/`buildServiceWorkerSource` erzeugt
  `dist-ui/sw.js` NUR beim Produktions-Build (kein Dev-Middleware-Pendant,
  um Vites HMR nicht zu stören). Strategie: Runtime-Caching statt fester
  Precache-Liste (Build-Hashes ändern sich pro Build, 303+ Artwork-Dateien
  wären als feste Liste unwartbar) — Navigation network-first mit
  Cache-Fallback, alles andere im `/deckbuilder/`-Scope cache-first,
  `Range`-Requests (Audio-Seeking) bewusst ausgenommen. Cache-Name enthält
  einen Build-Zeitstempel, alte Caches werden bei `activate` aufgeräumt.
  Registrierung in `src/ui/main.ts` nur wenn
  `import.meta.env.PROD && "serviceWorker" in navigator`.
- **Statistik-/Spielverlauf-Screen.** Neuer dauerhafter Spielverlauf in
  `localStorage` (Key `deckbuilder1.gameHistory`, gedeckelt auf die letzten
  100 Einträge), aus Sicht player1. Aufzeichnung über
  `store.ts#recordGameHistoryForEvent`, angeknüpft an dasselbe
  `gameEnded`-Event wie der bestehende Sieg-/Niederlage-Sound; Tutorial-
  Partien bewusst ausgeschlossen. Neuer `AppPhase`-Kind `"stats"`, neue
  Komponente `components/statsScreen.ts` (chronologischer Verlauf +
  Aggregation gesamt/pro Gegnertyp), neuer Button „Statistik" im
  Hauptmenü. Neuer Test `game-history.test.ts` (2 Fälle: Hotseat- und
  Bot-Partie).
- **Juice-Effekte mit Toggle.** Neuer Store-Zustand `effectsEnabled`
  (`isJuiceEnabled`/`toggleJuiceEnabled`, gleiches Persistenzmuster wie
  `sfxEnabled`), neue Toggle-Komponente `components/juiceToggle.ts` in
  derselben Statusleiste wie Musik-/SFX-/Bot-Speed-Toggle. Neue Funktion
  `store.ts#applyJuiceForEvent` mit drei neuen CSS-Effekten: `hit`
  (Zucken+Rot-Flash bei Schaden/Lebensverlust), `impact` (Gold-Puls beim
  tatsächlichen Auflösen eines Zaubers/einer Fähigkeit vom Stack, bewusst
  NICHT bei Fizzle/Countern), `death` (Aufleuchten/Ausblenden beim
  Erscheinen im Friedhof). Greift nur wenn eingeschaltet UND NICHT
  `prefers-reduced-motion`. Bestehende informationstragende Animationen
  bleiben bewusst ungekoppelt vom Toggle. Neuer Test `juice-toggle.test.ts`
  (4 Fälle).
- **Testzahlen laut Auftrag, vom documenter gegen den tatsächlichen
  Dateibaum per Glob/Grep gegengeprüft (kein eigener Shell-Zugriff für
  einen echten Testlauf in dieser Sweep-Session):** `npx tsc --noEmit`
  sauber, `npx vitest run` **48 Testdateien, 221 Tests grün, 1 bewusst
  übersprungen**, `npx vite build` erfolgreich (`dist-ui/` inkl. `sw.js`,
  `manifest.json`, `cards/artworks/` 7,9 MB) — per `Glob` bestätigt: exakt
  48 Testdateien im Baum (19 Engine + 5 KI, davon 1 die bereits vorher
  dokumentierte, noch uncommittete `hardBot-lethal.test.ts`, + 24 UI,
  davon die 2 neuen `game-history.test.ts`/`juice-toggle.test.ts`); die
  Summe 213 (v0.1.34-Stand) + 2 (`hardBot-lethal.test.ts`) + 2
  (`game-history.test.ts`) + 4 (`juice-toggle.test.ts`) = 221 stimmt exakt
  mit der gemeldeten Zahl überein.
- **documenter (dieser Sweep, 2026-08-02):** alle fünf Teile per direkter
  Code-Lektüre gegen `src/ui/*`/`vite.config.ts`/`package.json`/`public/*`
  verifiziert (nicht nur den Fertigstellungsbericht übernommen) — jede
  genannte Funktion/Datei/Klasse existiert exakt wie beschrieben, keine
  Abweichung gefunden. `docs/frontend-status.md` (neue Kopfzeile inkl.
  Uncommitted-Hinweis, neue „v0.1.35 auf einen Blick"-Kurzfassung,
  vollständiger neuer Detail-Abschnitt mit fünf Teilen + Verifikation,
  Struktur-Tabelle, „Was funktioniert" Punkte 6-9, „Nächste Schritte"
  Punkte 21-24), `docs/README.md` (diese Kopfzeile, Frontend-/
  Projekt-Setup-Zeilen der Statustabelle, dieser Absatz, „Weitere offene
  Punkte" Punkt 13 für den frontend-engineer), `docs/status.md` (neuer
  Meilenstein-Eintrag) auf den neuen Stand gehoben. `docs/rules-engine.md`,
  ...
  KI-Änderung in dieser Session — der KI-v2.2-Schritt selbst war bereits
  beim vorigen Mini-Sweep dokumentiert, hier nur in den zusammengeführten
  Testzahlen mitgezählt). Kein Code-/Test-Change durch den documenter
  selbst — reine Dokumentationsarbeit, kein eigener `npm test`/
  `npm run build`/`npx vitest run`/`npx vite build`-Lauf in dieser
  Sweep-Session (kein Shell-Werkzeug verfügbar) — alle Test-/Build-Zahlen
  stammen aus dem im Auftrag beschriebenen echten Lauf, per `Glob`/`Grep`
  gegengeprüft wie oben beschrieben, keine Abweichung gefunden.

**Seit dem letzten Sweep (2026-08-02, Frontend v0.1.35) zusätzlich —
hard-Bot Mana-Zurückhalten + 2-Ply gegen billige Gegenantwort (KI v2.3,
`docs/ai-status.md` Abschnitt 12), NOCH NICHT committet, keine
Engine-/Model-/Kartenpool-/Frontend-Änderung:**

- **Mana-Zurückhalten** (`src/ai/hardBot.ts#shouldHoldManaBack`, neue
  Konstante `MANA_HOLD_BACK_GAIN_CEILING = 1.0`): `hard` tappte im eigenen
  ...
  ohnehin nur marginal war — unterdrückt dann sowohl den proaktiven
  „weitere Quelle tappen"-Fallback als auch die Wahl eines marginalen
  Cast-/Activate-Kandidaten. Klar lohnende Optionen werden immer gespielt.
  Kartenpool-Fakt: 62 von 72 Spells sind `fast` — kein Randfall.
- **2-Ply gegen billige Gegenantwort**
  (`src/ai/hardBot.ts#evaluateCastCandidateEnd`/
  ...
  `fast`-Karte im gegnerischen Zug), werden jetzt zusätzlich bis zu zwei
  billige, statisch bestimmte Gegenantworten (Passen + die statisch beste
  gegnerische Option, gleiches Ranking wie sonst im Modul, KEIN eigenes
  Lookahead für die Antwort selbst) durchsimuliert und die für den Bot
  schlechtere verwendet — kein rekursiver Baum, Kosten K × (1-2) statt
  K × N. Beide Heuristiken nur für `hard`, `easy`/`medium` unverändert.
- **Neue Tests** `src/ai/__tests__/hardBot-mana-hold-back.test.ts` (2 Fälle,
  inkl. Gegenprobe mit unbezahlbaren Kosten) und
  `src/ai/__tests__/hardBot-cast-reply.test.ts` (2 Fälle, deterministische
  Falle: eine isoliert höher bewertete Option wäre nach gegnerischer Antwort
  tödlich riskant, die 2-Ply-Bewertung erkennt das — inkl. Gegenprobe, dass
  die reine 1-Ply-Bewertung ohne die neue Logik tatsächlich in die Falle
  tappen würde). Beide Testdateien vom documenter vollständig gelesen und
  gegen die beschriebenen Fallen abgeglichen, nicht nur den Bericht
  übernommen.
- **Performance/Test-Zahlen laut Bericht des ai-opponent-engineer, vom
  documenter NICHT selbst nachgerechnet** (kein Shell-Werkzeug in dieser
  Sweep-Session): hard-vs-hard über 20 Seeds mit einem für die Messung um
  Zauber ergänzten Testdeck (das bestehende `buildDeterministicDeck` in den
  Performance-Tests hatte laut Bericht bisher keine Zauber) — Median
  0,019 ms → 0,020 ms, p99 11,15 ms → 9,67 ms (keine relevante Regression),
  Max weiterhin < 34 ms. Stärkevergleich (`difficulty.test.ts`, unverändertes
  reines Kreaturen-Deck ohne Zauber, die neuen Heuristiken greifen dort also
  gar nicht): hard:medium 25:15, hard:easy 19:5 — identisch zur v2.2-Baseline.
- **Nachträglich per eigenem Testlauf bestätigt:** `npx vitest run src/ai`
  liefert 6 Testdateien grün + 1 übersprungen (7 gesamt), 26 Tests grün + 1
  übersprungen (27 gesamt) — die „26" aus dem ursprünglichen Bericht war
  korrekt, bezog sich aber auf Tests statt Testdateien (Details:
  `docs/ai-status.md` Abschnitt 12.4).
- **documenter (dieser Mini-Sweep, 2026-08-02):** `docs/ai-status.md` (neue
  ...
  `docs/README.md` (Kopfzeile, KI-Zeile der Statustabelle, dieser Absatz,
  „Weitere offene Punkte" Punkte 2+3 unten), `docs/status.md` (neuer
  Meilenstein-Eintrag) aktualisiert — `src/ai/hardBot.ts` vollständig sowie
  beide neuen Testdateien gegen den tatsächlichen Code gelesen, nicht nur
  den Fertigstellungsbericht übernommen. `docs/rules-engine.md`,
...
**Seit dem letzten Sweep (2026-08-02, Frontend v0.1.35 / KI v2.3) zusätzlich
— „Spielspeicher in der Partie" (Frontend v0.1.36, `docs/frontend-status.md`
gleichnamiger Abschnitt), NOCH NICHT committet, expliziter Nutzer-Auftrag,
keine Engine-/Model-/Kartenpool-/KI-Änderung:**

- **Single-Slot-Autosave + Fortsetzen.** Eine laufende Partie wird jetzt
  automatisch in einem einzelnen `localStorage`-Slot (Key
  `deckbuilder1.savedGame`) gesichert — `store.ts#autosaveGameForEvent`,
  aus derselben zentralen `processEvents`-Verarbeitung heraus wie
  `recordGameHistoryForEvent`/`applyJuiceForEvent` (v0.1.35), nach JEDEM
  state-verändernden Event neu geschrieben. Tutorial-Partien bewusst
  ausgeschlossen (gleiche Begründung wie beim Spielverlauf). Gelöscht bei
  `gameEnded` sowie beim Start einer neuen Partie über den normalen
  Gegner-Auswahl-/Deckbau-Ablauf (`store.ts#initGame`, außer im
  Tutorial-Pfad) — bewusst EIN durchgehender Slot statt eines
  Mehrfach-Speichersystems, daher stilles Überschreiben ohne
  Bestätigungsdialog.
- **Neue Store-Exporte** `hasSavedGame()`/`getSavedGameSummary()`/
  `resumeSavedGame()`. Letzteres restauriert `state`/`botControlledPlayers`/
  `botDifficulty` DIREKT aus der gespeicherten Payload, OHNE erneuten
  `engine.createGame`-Aufruf — verifiziert: die `RulesEngine` ist eine reine
  ...
  neuen zurück), die Factory-Closure hält außer dem `CardPool` keinen
  eigenen internen Zustand — direkte Restauration ist daher korrekt, kein
  erneuter Münzwurf/neue Starthände. Kein neuer `AppPhase`-Kind nötig.
- **Neue UI:** „Weiter spielen"-Button im Hauptmenü
  (`components/mainMenu.ts#resumeGameButton`), nur sichtbar bei
  vorhandenem Autosave, mit Zug-/Gegner-Untertitel (reuse der jetzt
  exportierten `statsScreen.ts#opponentLabel`).
- **Neuer Test** `src/ui/__tests__/game-save.test.ts` (4 Fälle: Roundtrip ab
  echtem App-„Reload" via `vi.resetModules()`, Tutorial-Ausschluss, Löschen
  bei `gameEnded`/Aufgeben, Bot-Steuerung/-Schwierigkeit-Rekonstruktion) —
  vom documenter vollständig gelesen und gegen die beschriebene
  Funktionsweise abgeglichen.
- **Testzahlen laut Auftrag, vom documenter per `Glob`/`Grep` gegen den
  tatsächlichen Dateibaum gegengeprüft (kein eigener Shell-Zugriff für
  einen echten Testlauf in dieser Sweep-Session):** `npx tsc --noEmit`
  sauber, `npx vitest run` **51 Testdateien (50 grün + 1 bewusst
  übersprungen), 229 Tests grün + 1 bewusst übersprungen (230 gesamt)** —
  rechnerisch konsistent mit 221 (v0.1.35-Stand) + 2
...
- **documenter (dieser Sweep, 2026-08-05):** `docs/frontend-status.md`
  (neue Kopfzeile, neue „v0.1.36 auf einen Blick"-Kurzfassung,
  vollständiger neuer Detail-Abschnitt „Spielspeicher in der Partie ...",
  Struktur-Tabelle, „Was funktioniert" Punkt 10, „Nächste Schritte" Punkt
  25), `docs/README.md` (Kopfzeile, Frontend-Zeile der Statustabelle,
  dieser Absatz) und `docs/status.md` (neuer Meilenstein-Eintrag)
  aktualisiert — der komplette Autosave-Abschnitt in `src/ui/store.ts`
  (inkl. `initGame`-Anpassung), `components/mainMenu.ts` sowie der neue
  Test wurden gegen den tatsächlichen Code gelesen, nicht nur den
  Fertigstellungsbericht übernommen; der zentrale Vertrag „`resumeSavedGame`
  ruft `createGame` NICHT erneut auf" wurde direkt im Code nachvollzogen.
  `docs/rules-engine.md`, `docs/engine-status.md`, `docs/ai-status.md`,
  `docs/cards/starter-set.md` waren NICHT Gegenstand dieses Sweeps (keine
  ...
  verfügbar) — alle Test-/Build-Zahlen stammen aus dem im Auftrag
  beschriebenen echten Lauf, per `Glob`/`Grep` gegengeprüft wie oben
  beschrieben, keine Abweichung gefunden. Zusätzlich geprüft (Auftrags-
  Randbemerkung zur Zielgruppen-Framing): in keiner der drei betreuten
  Dateien (`docs/README.md`, `docs/frontend-status.md`, `docs/status.md`)
  findet sich eine „für Kinder"/kindgerecht-Framing des Deckbuilders — s.
  dortiger Grep-Befund (die einzigen Treffer auf „Kinder" im gesamten
  `docs/`-Baum sind DOM-„Kinder" im Sinne von Kindelementen, kein
  Zielgruppenbezug) — daher bewusst keine solche Formulierung eingeführt
  oder bestätigt.

**Seit dem letzten Sweep (2026-08-05, Frontend v0.1.36) zusätzlich — zwei
voneinander unabhängige Frontend-Features (v0.1.37,
`docs/frontend-status.md` gleichnamiger Abschnitt), NOCH NICHT committet,
keine Engine-/Model-/Kartenpool-/KI-Änderung:**

- **Auto-Tap-Komfort.** Löst den seit `docs/rules-engine.md` 9.5
  dokumentierten „kein Auto-Tap ... kommt später"-Punkt (in diesem Sweep dort
  entsprechend aktualisiert). Behebt zusätzlich einen real gemeldeten Bug:
  manuelles Vor-Tappen konnte 2 gleichfarbige Quellen für eine Karte
  verbrauchen, die nur 1 Pip + 1 generisch brauchte, und dadurch eine zweite
  identische Karte unbezahlbar machen, obwohl insgesamt genug Gesamt-Mana da
  war. Neue `store.ts`-Funktionen `hypotheticalStateWithExtraMana`
  (Refactoring — gemeinsam mit dem bestehenden `hasRealPriorityChoice`
  genutzt statt einer zweiten unabhängigen Kopie), `castCandidatesForHandCard`,
  `AUTO_TAP_COLOR_ORDER`, `manaAbilityColor`, `selectAutoTapSources`,
  `autoTapActionsForCast`, verdrahtet in `dispatch()`: farbige Pips werden
  zuerst aus gleichfarbigen Quellen gedeckt, generische Kosten bevorzugt aus
  farblosen Quellen (mirrort `engine/mana.ts#payCost`, rückwärts auf die
  Quellenauswahl angewandt). Nur menschliche Spieler betroffen, Bot-
  Entscheidungslogik (`src/ai/*`) unverändert; X-Kosten-Zauber bewusst
  ausgenommen (aktuell kein Kartenpool-Fall). Neuer Test
  `auto-tap-mana.test.ts` (1 Fall, reproduziert exakt das gemeldete
  Szenario).
- **Friedhof einklappen.** Ein nicht-leerer Friedhof zeigte vorher jede
  Karte einzeln in voller Größe. Jetzt EINE eingeklappte Stapel-Kachel
  (neue Datei `components/graveyardStack.ts#graveyardStackTile`, zeigt das
  ECHTE Artwork der obersten Karte statt eines Kartenrückens — der Friedhof
  ist, anders als die verdeckte Hand, öffentliche Information — plus
  Zahl-Badge, reuse von `.hand-card-hidden-stack-multi`/`-count` aus der
  v0.1.35-Hand-Stapel-Kachel) statt jeder Karte einzeln. Klick öffnet ein
  scrollbares Popover (`graveyardPopoverPanel`, reuse des
  `.tutorial-help-backdrop`/`-panel`-Musters von `keywordGlossaryPanel`) mit
  der vollständigen Liste, Schließen per Button oder Backdrop-Klick. Leerer
  Friedhof unverändert. Neue Store-Exporte `getOpenGraveyardPopover`/
  `toggleGraveyardPopover`/`closeGraveyardPopover`. Neuer Test
  `graveyard-stack.test.ts` (2 Fälle).
- **Testzahlen laut Auftrag, vom documenter per `Read`/`Glob`/`Grep` gegen
  den tatsächlichen Dateibaum UND den tatsächlichen Testcode gegengeprüft
  (kein eigener Shell-Zugriff für einen echten Testlauf in dieser
  Sweep-Session):** `npx tsc --noEmit` sauber, `npx vitest run` **53
  Testdateien (52 grün + 1 bewusst übersprungen), 232 Tests grün + 1 bewusst
  übersprungen (233 gesamt)** — rechnerisch konsistent mit 229
  (v0.1.36-Stand, 51 Dateien) + 1 (`auto-tap-mana.test.ts`) + 2
  (`graveyard-stack.test.ts`) = 232.
- **documenter (dieser Sweep, 2026-08-06):** jede genannte
  Funktion/Datei/Klasse per `Read`/`Grep` direkt gegen `src/ui/store.ts`,
  `src/ui/render.ts`, die neue Datei `src/ui/components/graveyardStack.ts`,
  `src/ui/style.css` sowie beide neuen, vollständig gelesenen Testdateien
  verifiziert (nicht nur den Fertigstellungsbericht übernommen) — keine
  Abweichung gefunden. `docs/frontend-status.md` (neue Kopfzeile, neue
  „v0.1.37 auf einen Blick"-Kurzfassung, vollständiger neuer Detail-Abschnitt
  „Auto-Tap-Komfort + Friedhof einklappen (v0.1.37)", Struktur-Tabelle,
  „Was funktioniert" Punkte 11+12, „Nächste Schritte" Punkte 26-28),
  `docs/rules-engine.md` (Abschnitt 9.5 aktualisiert — der „kommt
  später"-Satz ist jetzt als erledigt markiert, mit Verweis auf
  `docs/frontend-status.md`; kein Engine-Code betroffen), `docs/README.md`
  (Kopfzeile, Frontend-Zeile der Statustabelle, dieser Absatz, „Weitere
  offene Punkte" Punkt 15) und `docs/status.md` (neuer Meilenstein-Eintrag)
  aktualisiert. `docs/engine-status.md`, `docs/ai-status.md`,
  `docs/cards/starter-set.md` waren NICHT Gegenstand dieses Sweeps (keine
  Engine-/Model-/Kartenpool-/KI-Code-Änderung in dieser Session). Kein
  Code-/Test-Change durch den documenter selbst — reine
  Dokumentationsarbeit, kein eigener `npm test`/`npm run build`/
  `npx vitest run`-Lauf in dieser Sweep-Session (kein Shell-Werkzeug
  verfügbar) — alle Test-/Build-Zahlen stammen aus dem im Auftrag
  beschriebenen echten Lauf, per `Glob`/`Grep` gegengeprüft wie oben
  beschrieben, keine Abweichung gefunden.
...
12. ~~**UI-Testsuite (`src/ui/__tests__/*`) zeitweise gar nicht
    ausführbar**~~ **erledigt in v0.1.34 (documenter-Notiz 2026-08-02):**
    `jsdom` in `package.json` exakt auf `26.1.0` gepinnt (behebt einen
    ESM/CJS-Interop-Fehler mit `html-encoding-sniffer`/`@exodus/bytes`, s.
    „Seit dem letzten Sweep (2026-07-22)" oben). Ein echter `npx vitest run`
    lief danach über die gesamte Suite: **44 von 45 Testdateien / 213 von
    214 Einzeltests grün** — ersetzt die vorherige „177/177"-Angabe aus
    Punkt 10 als aktuellen Gesamtstand. Weiterhin offen: eine echte
    Browser-/Screenshot-Verifikation der Optik (s. Punkt 10) — daran hat
    diese Session nichts geändert, kein Browser-/Computer-Use-Werkzeug
    verfügbar.
13. **Neu (documenter-Notiz 2026-08-02, v0.1.35, NOCH NICHT committet):**
    fünf unabhängige Änderungen (kompaktere KI-Hand-Stapel-Optik,
    Karten-Artwork-Kompression auf 480px-WebP, PWA-/Offline-Service-Worker,
    Statistik-/Spielverlauf-Screen, Juice-Effekte mit eigenem Toggle) — s.
    `docs/frontend-status.md` v0.1.35 für Details, alle fünf gegen den
    tatsächlichen Code verifiziert. Offen: v0.1.35 committen (analog zu
    Punkt 9 bei v0.1.20/v0.1.21); echte Browser-/Screenshot-Verifikation der
    Artwork-Kompression und des Hand-Stapels; ein echter Offline-/
    Lighthouse-PWA-Test des neuen Service Workers; eine „Verlauf löschen"-
    Funktion für den Statistik-Screen fehlt bisher (kleine, naheliegende
    Ergänzung, s. `docs/frontend-status.md` Nächste-Schritte-Punkt 24).
14. **Neu (documenter-Notiz 2026-08-05, v0.1.36, NOCH NICHT committet,
    expliziter Nutzer-Auftrag „Spielspeicher in der Partie"):**
    Single-Slot-Autosave (`localStorage`-Key `deckbuilder1.savedGame`) +
    „Weiter spielen"-Button im Hauptmenü — s. `docs/frontend-status.md`
    Abschnitt „Spielspeicher in der Partie ..." für Details, gegen den
    tatsächlichen Code verifiziert (inkl. des Vertrags, dass
    `resumeSavedGame` die `RulesEngine` NICHT erneut über `createGame`
    aufruft). Offen: v0.1.35 UND v0.1.36 committen; keine manuelle
    „Autosave verwerfen"-Option im UI (rein automatisch verwaltet, bewusste
    Design-Entscheidung); echte Browser-Verifikation des Reload-Verhaltens
    (bisher nur per `vi.resetModules()`-simuliertem „Reload" im neuen Test
    abgedeckt).
15. **Neu (documenter-Notiz 2026-08-06, v0.1.37, NOCH NICHT committet):**
    Auto-Tap-Komfort (löst `docs/rules-engine.md` 9.5 „kommt später" ein UND
    einen real gemeldeten Mana-Tap-Bug) + Friedhof einklappen (Stapel-Kachel
    + Popover statt jeder Karte einzeln) — s. `docs/frontend-status.md`
    Abschnitt „Auto-Tap-Komfort + Friedhof einklappen (v0.1.37)" für
    Details, beide gegen den tatsächlichen Code verifiziert. Offen: v0.1.35,
    v0.1.36 UND v0.1.37 committen; Auto-Tap-Komfort unterstützt bewusst
    keine X-Kosten-Karten (aktuell kein Kartenpool-Fall); echte
    Browser-/Screenshot-Verifikation beider Features steht aus (bisher nur
    per jsdom-Klick-Test abgedeckt).
...
2. Kein echtes Multi-Ply-Minimax/MCTS (hard bleibt 1-Ply mit
   ...
   `evaluateState`) ist dafür vorbereitet. **Präzisiert seit v2.2
   (2026-08-02):** ein gezielter Lethal-Sonderfall (`findLethalAction`)
   schließt jetzt die konkrete Schwäche „Zauber + Angriff wäre lethal,
   isoliert bewertet aber schlechter" — kein allgemeines Mehr-Ply-Lookahead,
   s. `docs/ai-status.md` Abschnitt 11. **Weiter präzisiert seit v2.3
   (2026-08-02, uncommitted):** die Top-K-Cast-/Activate-Shortlist wird
   zusätzlich gegen bis zu zwei billige, statisch bestimmte gegnerische
   Antworten geprüft (`evaluateCastCandidateEnd`) — ein eng begrenzter
   2-Ply-Ausschnitt (K × (1-2) statt K × N), keine rekursive Suche, s.
   `docs/ai-status.md` Abschnitt 12.
3. Kein Instant-Speed-Spiel in irgendeiner Stufe. **Teilweise behoben seit
   v2.3 (2026-08-02, uncommitted):** `hard` hält im eigenen Main-Phase-Fenster
   jetzt gezielt Mana zurück, wenn eine bezahlbare, „typischerweise
   reaktive" `fast`-Karte sonst unbezahlbar würde (`shouldHoldManaBack`) —
   nur dieser eng umrissene Grenzfall, kein allgemeines „immer Mana für
   Instants reservieren"; kein Bot spielt weiterhin proaktiv Instant-Speed
   im fremden Zug, s. `docs/ai-status.md` Abschnitt 12.
