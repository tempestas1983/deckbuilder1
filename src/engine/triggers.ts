...
export function fireSelfCombatTrigger(
  ...
  pool: CardPool,
  instanceId: InstanceId,
  kind: "onAttackDeclared" | "onBlockDeclared" | "onDealtCombatDamageToPlayer" | "onDamageReceived",
): void {
  forEachTriggeredAbility(state, pool, instanceId, (index, trigger) => {
...
function buildTriggerStackObject(trigger: PendingTrigger, chosenTargets: ChosenTarget[]): Omit<
  Extract<StackObject, { kind: "triggeredAbility" }>,
  "id"
> {
  return {
...
export function pushResolvedTriggerToStack(
  ...
  const obj: StackObject = { id: stackObjectId, ...buildTriggerStackObject(trigger as PendingTrigger, chosenTargets) };
