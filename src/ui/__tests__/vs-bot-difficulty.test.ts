  ...
  it(
    ...
    async () => {
      ...
      while (getState().winner === undefined && iterations < MAX_ITERATIONS) {
        ...
        // Bugfix (s. docs/frontend-status.md, hasRealPriorityChoice): sobald
        // der Mana-Pool leer ist, aber ein ungetapptes Terrain zusammen mit
        // einer teureren Handkarte theoretisch etwas bezahlbar machen WÜRDE,
        // zeigt render.ts#statusBar jetzt korrekt das auffällige
        // Spotlight-Banner statt des kleinen ".btn-pass" (s. dortiger
        // Kommentar "spotlightAlreadyShown") - Spieler 1 spielt in diesem Test
        // bewusst passiv (s. vs-bot.test.ts-Vorbild), der
        // Banner-"Überspringen"-Button löst exakt dieselbe passPriority-Aktion
        // aus wie ".btn-pass" oben.
